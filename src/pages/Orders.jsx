...
const Orders = () => {
  ...
  const showUserOrders= async () => {
    ...
    const response = await axios
      .get("http://websitebuild.herokuapp.com/api/Customer/all-orders",{headers:{shop:"gamal", "auth-token":user}})
      .catch((err) => {
        console.log("Err: ", err);
        setLoading(false)
      });
  ...
  const RefundOrder= async (w) => {
    ...
    setLoading(true)
    const response = await axios
      .post("http://websitebuild.herokuapp.com/api/Customer/refund_request/",{id,reason},{headers:{shop:"gamal", "auth-token":user}})
      .catch((err) => {
